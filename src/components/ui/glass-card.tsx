...
const GlassCard = React.forwardRef<HTMLDivElement, GlassCardProps>(
  (
    ...
    return (
      <div
        ref={ref}
        className={`group h-[300px] w-full max-w-[290px] [perspective:1000px] ${className}`}
        {...props}
      >
        <div className="relative h-full rounded-[50px] bg-gradient-to-br from-[#a30005] to-[#800004] shadow-2xl transition-all duration-500 ease-in-out [transform-style:preserve-3d] group-hover:[box-shadow:rgba(205,0,7,0.3)_30px_50px_25px_-40px,rgba(205,0,7,0.1)_0px_25px_30px_0px] group-hover:[transform:rotate3d(1,1,0,30deg)]">
          ...
          <div className="absolute bottom-5 left-5 right-5 flex items-center justify-between [transform-style:preserve-3d] [transform:translate3d(0,0,26px)]">
            <div className="flex gap-2.5 [transform-style:preserve-3d]">
              {[
                ...
                >
                  <Icon className="h-4 w-4 stroke-black transition-colors group-hover/social:stroke-white group-active/social:stroke-black" />
                </button>
              ))}
            </div>
