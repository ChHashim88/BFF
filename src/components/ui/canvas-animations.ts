      ...
      dotRings.forEach((ring) => {
        for (let i = 0; i < ring.count; i++) {
          ...
          if (Math.abs(dist) < waveThickness / 2)
            pF = Math.max(0, Math.cos((dist / (waveThickness / 2)) * (Math.PI / 2)));
          let angle = bAngle;
          if (pF > 0.01) angle += pF * Math.sin(time * rotSpeed + i * 0.5) * rotMag;
          const s = 1.5 + pF * 1.8,
            o = 0.2 + pF * 0.7;
    ...
    return (timestamp) => {
      ...
      dotRings.forEach((ring, rIdx) => {
        ...
        for (let i = 0; i < ring.count; i++) {
          ...
          const pPhase = time * 1.5 - ring.radius / 20 + (i / ring.count) * Math.PI,
      ...
      dotRings.forEach((ring, rIdx) => {
        for (let i = 0; i < ring.count; i++) {
          ...
          for (let b = 0; b < numBands; b++) {
            const bandCenterY =
              ((time * bandSpeed + b * (totalPath / numBands)) % totalPath) - bandWidth / 2;
            const distToBand = Math.abs(y - bandCenterY);
            let inf = 0;
            if (distToBand < bandWidth / 2)
              inf = Math.max(0, Math.cos((distToBand / (bandWidth / 2)) * (Math.PI / 2)));
    ...
    return (timestamp) => {
      ...
      ringData.forEach((ring) => {
        ...
        for (let i = 0; i < ring.count; i++) {
          ...
          ctx.fillStyle = THEME_FILL(opacity);
          ctx.fill();
        }
      });
    };
    ...
    return (timestamp) => {
      ...
      arms.forEach((arm) => {
        arm.dots.forEach((dot) => {
          ...
          if (dist < waveLen / 2)
            pInf = Math.max(0, Math.cos((dist / (waveLen / 2)) * (Math.PI / 2)));
  ...
  radiatingLineScan: ((ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
    ...
    return (timestamp) => {
      ...
      for (let i = 0; i < numLines; i++) {
        ...
        ctx.strokeStyle = THEME_FILL(0.1);
        ctx.lineWidth = 0.5;
        ctx.stroke();
