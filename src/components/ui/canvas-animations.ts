...
// --- ANIMATION LOGIC ---
const THEME_FILL = (opacity: number) => `rgba(205, 0, 7, ${Math.max(0, Math.min(1, opacity))})`;
...
export const animationLogic = {
    pulseWave: ((ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
        ...
        return (timestamp) => {
            ...
            ctx.beginPath(); ctx.arc(centerX, centerY, 2, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(0.9); ctx.fill();
            dotRings.forEach((ring, rIdx) => {
                for (let i = 0; i < ring.count; i++) {
                    ...
                    ctx.beginPath(); ctx.arc(x, y, 2, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(o); ctx.fill();
                }
            });
        };
    }) as SetupFunction,
    pulseWaveShockwave: ((ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
        ...
        return (timestamp) => {
            ...
            const dT = timestamp - lastTime; lastTime = timestamp; time += dT * 0.001;
            ctx.clearRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
            ctx.beginPath(); ctx.arc(centerX, centerY, 1.5, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(0.8); ctx.fill();
            ...
            dotRings.forEach(ring => {
                for (let i = 0; i < ring.count; i++) {
                    ...
                    ctx.beginPath(); ctx.arc(x, y, s, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(o); ctx.fill();
                }
            });
        };
    }) as SetupFunction,
    ...
    pulseWaveBreathingGrid: ((ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
        ...
        return (timestamp) => {
            ...
            dots.forEach(dot => {
                ...
                ctx.beginPath(); ctx.arc(dot.x, dot.y, s, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(o); ctx.fill();
            });
        };
    }) as SetupFunction,
    flowingEnergyBands: ((ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
        ...
        return (timestamp) => {
            ...
            const dT = timestamp - lastTime; lastTime = timestamp; time += dT * 0.001;
            ctx.clearRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
            ctx.beginPath(); ctx.arc(centerX, centerY, 1.5, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(0.7); ctx.fill();
            dotRings.forEach((ring, rIdx) => {
                for (let i = 0; i < ring.count; i++) {
        ...
        return (timestamp) => {
            ...
            dotRings.forEach((ring, rIdx) => {
                for (let i = 0; i < ring.count; i++) {
                    ...
                    ctx.ellipse(0, 0, rX, rY, 0, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(o); ctx.fill(); ctx.restore();
                }
            });
        };
    }) as SetupFunction,
        ...
        return (timestamp) => {
            ...
            ringData.forEach(ring => {
                ...
                for (let i = 0; i < ring.count; i++) {
                    ...
                    const x = centerX + Math.cos(angle) * ring.radius, y = centerY + Math.sin(angle) * ring.radius;
                    ctx.beginPath(); ctx.arc(x, y, size, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(opacity); ctx.fill();
                }
            });
        };
    }) as SetupFunction,
    ...
    radiatingLineScan: ((ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
        ...
        return (timestamp) => {
            ...
            const dT = timestamp - lastTime; lastTime = timestamp; time += dT * 0.001;
            ctx.clearRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
            ctx.beginPath(); ctx.arc(centerX, centerY, 2, 0, Math.PI * 2); ctx.fillStyle = THEME_FILL(0.7); ctx.fill();
            ...
            for (let i = 0; i < numLines; i++) {
                ...
                ctx.strokeStyle = THEME_FILL(0.1); ctx.lineWidth = 0.5; ctx.stroke();
                ...
                if (scanStart < scanEnd) {
                    ...
                    grad.addColorStop(0, THEME_FILL(0));
                    grad.addColorStop(0.5, THEME_FILL(0.85 * pulse));
                    grad.addColorStop(1, THEME_FILL(0));
...
export const useCanvasAnimation = (
  ...
  useEffect(() => {
    ...
    let render: AnimateFunction | null = null;
    ...
    const resizeCanvas = () => {
      ...
      if (animateFunction) {
        render = animateFunction(ctx, width, height);
      }
    };
