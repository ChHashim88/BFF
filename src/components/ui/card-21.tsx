...
const DestinationCard = React.forwardRef<HTMLDivElement, DestinationCardProps>(
  ({ className, imageUrl, location, flag, stats, description, readMoreContent, href, themeColor, ...props }, ref) => {
          ...
          >
            ...
            <div 
              className={cn(
                ...
                isExpanded ? "opacity-0 pointer-events-none absolute inset-x-6 inset-y-6 scale-[0.95] overflow-hidden" : "opacity-100 scale-100"
              )}
            >
              <h3 className="text-subtitle min-h-[56px] mb-2 drop-shadow-md">
                {location} {flag && <span className="ml-1">{flag}</span>}
              </h3>
              {stats && <p className="text-sm text-white/90 mt-1 font-medium drop-shadow-sm">{stats}</p>}
              
              {description && (
                <div className="text-nav text-white/90 mt-2 space-y-3">
                  {description}
                </div>
              )}
            ...
            {readMoreContent && (
              <div 
                className={cn(
                  ...
                  !isExpanded ? "opacity-0 pointer-events-none absolute inset-x-6 inset-y-6 scale-[1.05] overflow-hidden" : "opacity-100 scale-100"
                )}
              >
    ...
    const cardWrapperClass = "relative block w-full h-full rounded-3xl overflow-hidden shadow-2xl transition-all duration-500 ease-in-out group-hover:scale-[1.01] group-hover:shadow-[0_0_80px_-15px_hsl(var(--theme-color)/0.5)]";
